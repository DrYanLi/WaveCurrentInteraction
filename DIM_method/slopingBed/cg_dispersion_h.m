function [tau_sh,tau_lin,cg_sh,cg_lin,k_v]=cg_dispersion_h()
%% shear current data
Frh       = 0.0;
theta     = 1* pi; 
N         = 127;
...
omega_nvs = sqrt(tanh(k_i.*h_i)*g.*k_i); 
cg_nvs    = sqrt(tanh(k_i.*h_i).*g./(k_i))/2.*(1+2*(k_i.*h_i)./sinh(2*(k_i.*h_i)))+Fr_shear.*sqrt(g)*cos(theta);
% contourf(k_i,h_i,omega_v)
subplot(1,3,1)
contourf(k_i,h_i,cg_Q)
subplot(1,3,2)
contourf(k_i,h_i,cg_nvs)
subplot(1,3,3)

mincg =  min(min(cg_Q./cg_nvs));
maxcg = max(max(cg_Q./cg_nvs));
stepcg = (maxcg-mincg)/9;
hv = mincg:stepcg:maxcg;
[c,h]=contourf(k_i,h_i,cg_Q./cg_nvs,hv)
